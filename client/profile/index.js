...
// Մենյու ակտիվ էջի լոգիկա
document.querySelectorAll(".menu_list a").forEach((link) => {
  link.parentElement.classList.remove("active");
});

// 🧑‍💻 Բեռնված օգտատիրոջ տվյալները
async function getCurrentUser() {
  const respons = await fetch("/currentUser");
  const result = await respons.json();
  const { favorites } = result;
  ...
  if (result.firstname) {
    ...
    renderFavorites(favorites, result.email); // ⬅ օգտագործում ենք ներքևում գրած ֆունկցիան
...
// 🔁 Ընդհանուր ֆունկցիա՝ քարտերի տպման համար (օգտագործվում է նաև փնտրման մեջ)
function renderFavorites(favorites, email) {
  ...
  favorites.forEach((fav) => {
    ...
    content += `<button class="delete-btn" onclick="deleteFavorite('${email}', '${fav.id}')">Ջնջել</button></div>`;
